...
!!! note "评语"
    论文排版较为美观。实现了多种FFT算法并做了对比。不足之处在于：（1）没有和MATLAB自带的FFT程序对比，没有切题；（2）对比流于表面，如果能深入分析各种算法对于什么样的序列更有优势，并从理论和实验去证明，会更好。
    （其实写了……但是写的确实不够多。猜测是因为无关篇幅写太长了。不过这篇课设我还是很用心的，我认为可以用来帮助大家理解和手撕FFT算法，毕竟我也是纯手写的。）
